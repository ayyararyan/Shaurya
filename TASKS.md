...
| ID | Task | Harvest from | Depends on | Status |
|---|---|---|---|---|
| CON-01 | Snapshot/tape row schema: BBO, depth, timestamps, quality flags | Market Making `surface_snapshots_<run_id>.csv` schema, `monday_v1/snapshot_tape.py` | — | **Live verified 2026-08-18 for the original capture fields; DAT-14 extension Dry-run verified 2026-08-19.** Schema `1.1.0` adds raw print/quote inputs, both BBO-leg receive times, quote age/bound, versioned side/alignment, degraded reason and coalesced flag while loading retained `1.0.0` rows. The original 20-level BBO/depth, timestamps, sequences and flags remain live-verified in run `sha-20260818T055709.463701Z-ea8228c8`; the new trade fields have not yet been live-captured |
...
| DAT-11 | **Bisect the exact 20-level per-message instrument ceiling.** 2026-08-18's four live tests pinned it somewhere between 52 (worked) and 206 (rejected outright) but ran out of trading-hours window to narrow it further. Needed to turn "50-80+ connections" into an exact connection-count plan | `scripts/dat09_concurrency_probe.py` (extend with a binary-search loop) | DAT-09 | **Live verified 2026-08-19: exact observed ceiling = 50 instruments per message; solo-rate addendum identifies an observed per-instrument delivery cap near 8 packets/s.** Fresh sockets and exactly one message per candidate; NIFTY/BANKNIFTY front futures led every prefix as liquid controls. Counts 2, 27, 40, 46, 49 and 50 were accepted with every selected instrument receiving packets; 51 and 53 were rejected wholesale with zero packets. The tested acceptance table is monotone. This directly corrects the prior-day claim that 52 worked: a fresh 52 boundary run also received zero packets. In two later fresh-socket solo NIFTY runs, 116 and 120 packets arrived in 15 s versus 116 inside the 50-instrument run. Removing 49 instruments did not materially lift the rate, so the predeclared addendum conclusion is `per-instrument-rate-cap`, implying an approximately 125–129 ms delivered/coalesced-view bound for D23 rather than shared-socket bandwidth collapse. Underlying within-interval exchange events remain unidentified. Dated evidence: `docs/live-evidence/DAT-11-2026-08-19.md` |
...
| `MODULE_SPEC.md` | **Drafted 2026-08-18; extended through D24 on 2026-08-19** — root index plus 13 per-component specifications under `docs/module-spec/`; all 109 non-dropped task IDs mapped exactly once to stable `REQ-*` rows with code/test/output targets |
...
| Tasks in progress | DAT-11/12/13 are Live verified; DAT-14 is Live verified for two front-month futures/depth20/one morning window; DAT-15 is Live verified for the retained eight-tape sample with the stressed cross-tier and missing option/midday coverage explicit. INF-02/05/07/09 are tested; INF-06 is blocked on exact approved destinations for 8 Drive-hosted files. CON-02/03/04/05/06/07/09 are tested; CON-01/08 and the Dhan slice of CON-05 retain live evidence. DAT-01/DAT-02/DAT-10-15 are live-verified at their stated scopes; DAT-03/04/07 are tested; DAT-05/06 are dry-run verified; DAT-08 is dropped; DAT-09 planning/pooling is tested with the final strike-band/connection-count owner choice still open |

**Immediate next action:** DAT-11 through DAT-15 market-hours work is landed. Use the measured
50-instrument ceiling, socket-scoped 50+50 reconnect result, and 200-level first-subscription
throttle when Aryan chooses the final strike bands and connection count. Do not infer that the
NIFTY test future is itself the capture universe, and do not generalize DAT-15 beyond its explicit
instrument/tier/time coverage. Other components continue in the recorded dependency order.
