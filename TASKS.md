...
| SIG-03Y | **Cont-Kukanov-Stoikov level-one OFI → future futures returns, depth-controlled exploratory scan (`X-CKS-L1-OFI-DAT20-04`).** Directed by Aryan by voice 2026-08-19 ~19:56 IST: build the canonical CKS best-quote event increment, measure the identifiable event intensities, form net pressure, and quantify how much of the subsequent futures mid change it explains **while controlling for futures order-book depth**. 25 cells (5 accumulation windows × 5 return horizons, 0.5 s causal gap), six models per cell — OFI alone, causal `log1p` level-one depth alone, depth+OFI, depth-scaled pressure, depth+pressure, and a separately labelled spread/microprice robustness arm — plus a direct comparison arm carrying `X-OFI-DAT20-03`'s price-keyed top-10 construction against the same depth control. Eight-component auditable transition decomposition. **Not `H-SIG21`; not confirmatory; no economic claim.** | New; directed by Aryan 2026-08-19 under D35 | SIG-03X, SIG-09/11/15, DAT-20 | **Dry-run verified — machinery Level 3; empirical content exploratory and weaker than `X-OFI-DAT20-03`.** Complete 25-cell grid over the same 5,204 observations. Raw level-one OFI adds at most **+1.44 pp** of held-out R² over the depth control (1 s → 2 s), positive in 11/25 cells. **Depth scaling clearly helps — 22/25 cells —** with a best increment of **+6.01 pp** (2 s → 2 s, pressure-only OOS R² 6.31%). But only 2/25 raw and 1/25 pressure cells clear all three dependence checks, the one substantive survivor **flips coefficient sign between the two tapes** (+1.72 vs −0.32 ticks/SD), and its best pressure cell scores higher out of sample than in sample (5.30% vs 1.36%). `X-OFI-DAT20-03`'s lead survives this scan's independent depth control at **+7.16 pp** while level one contributes **−0.54 pp** at that cell with a negative coefficient — confirming the earlier lead is a levels-2–10 phenomenon and not a best-quote one. Object is dominated by best-ask price relocation inside a **100–134 tick median spread**; same-price displayed size changes are only 19.1% of absolute contribution. Gross arrivals and gross cancellations remain **unidentified**; the coalesced trade fields do not deliver a clean execution split. Verdict: exploratory observation, not a signal. Report: `docs/CKS-L1-OFI-2026-08-19.md`. |
