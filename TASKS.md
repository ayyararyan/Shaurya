...
**Next substantive step (evening 2026-08-19, set explicitly by Aryan):** return to the
story-by-story signal-mining debate and take the next taxonomy cell, **price-path-derived objects
(`PP`, `SIG-04`)**. Start with what each object influences and why that changes a maker's joint
quoting configuration; then bind every accepted hypothesis under `D29`/`METHOD.md`. The book-state
detour was useful and is closed at the estimand level by `D30`; do not let controller design
displace the signal-mining discussion again. `SIG-21` remains open and must still be pre-registered
before its deep-book outcomes are inspected, but it is not the next conversation Aryan requested.
...
| SIG-21 | **Deep-book anomaly → future-price response (`H-SIG21`).** Test whether rare disturbances in the normally quiet far depth200 ladder forecast later NIFTY-futures mid-price movement. The event definition must be **price-keyed, causal and pre-registered before outcomes are inspected**: additions, removals, relocations, displayed-quantity shocks and order-count shocks are separated; mechanical level-index cascades and whole-ladder window slides are excluded; unusualness is measured against a past-only baseline conditional on side, distance from the same-side best quote, time of day and liquidity/regime. Outcomes are future mid-price returns at a small, theory/evidence-narrowed set of `D27`-admissible horizons; contemporaneous response is reported separately from predictive response. Use non-overlapping event clusters or dependence-robust inference, matched quiet controls, explicit effective sample size and one `SIG-12` family across event type × side × distance × threshold × horizon. Report the full response distribution, sign, peak timing and reversion — not merely significance. **The two 11-minute `DAT-20` runs are feasibility evidence only and cannot establish predictability; the pre-registration must specify a minimum multi-session/multi-regime sample and ex-ante MDE before testing.** | New; motivated by D28 and enabled by DAT-20 | DAT-05, DAT-10, DAT-20, SIG-01/02/04/09/11/12/14/19, D20, D22, D27, D28 | **Not started — exact next research task. First action is the pre-registration and sample/power gate; no price-response result may be inspected first** |
...
| `MODULE_SPEC.md` | **Drafted 2026-08-18; extended through D30 on 2026-08-19** — root index plus 13 per-component specifications under `docs/module-spec/`; all non-dropped task IDs map to stable `REQ-*` rows with code/test/output targets. `SIG.md` now binds D29's hypothesis-resolution method and D30's joint configuration estimand; dropped `DAT-08`/`EXE-07/08` intentionally have no requirement |
...
| Tasks in progress | DAT-11/12/13 are Live verified; DAT-14/15/16/17/20 are Live verified at their stated scopes. `DAT-20` establishes reliable deep-book observability and quiet-time skips, not rare-event predictability. `SIG-21` remains open and must be registered before inspecting its outcomes. **The next requested discussion is the `PP` / `SIG-04` signal class under D29, after which its accepted hypotheses enter the claim ledger.** INF-02/05/07/09 are tested; INF-06 is blocked on exact approved destinations for 8 Drive-hosted files. CON-02/03/04/05/06/07/09 are tested; CON-01/08 and the Dhan slice of CON-05 retain live evidence. DAT-03/04/07 are tested; DAT-05/06 are dry-run verified; DAT-08 is dropped; DAT-09 planning/pooling is tested with the final strike-band/connection-count owner choice still open |

**Immediate next action:** resume the `PP` / `SIG-04` signal-mining debate this evening. Separately,
write and commit the `SIG-21` pre-registration **before** inspecting any
future-price response. It must bind the causal anomaly definition, event types, quiet-region
normalisation, admissible horizon set, matched controls, dependence/overlap rule, full multiplicity
family, minimum multi-session/multi-regime sample and ex-ante MDE. The retained `DAT-20` tapes may
be used to verify parser/event construction only; their future returns remain unopened until the
pre-registration and sample/power gate exist.
