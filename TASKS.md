...
**Next substantive step (updated 2026-08-19):** pre-register `SIG-21` before inspecting any
future-price response: bind the causal deep-book anomaly definition, event taxonomy, admissible
horizons, dependence treatment, multiple-testing family, minimum detectable effect, and minimum
multi-session sample. `DAT-20` proves the feed can observe the deep book; it does not yet prove
that rare deep-book disturbances predict price.
...
| `MODULE_SPEC.md` | **Drafted 2026-08-18; extended through D28 and SIG-21 on 2026-08-19** — root index plus 13 per-component specifications under `docs/module-spec/`; all 116 non-dropped task IDs mapped exactly once to stable `REQ-*` rows with code/test/output targets. A 2026-08-19 audit repaired four pre-existing unmapped follow-up rows (`DAT-18/19/20`, `ANL-05`) before adding `SIG-21`; dropped `DAT-08`/`EXE-07/08` intentionally have no requirement |
...
| Tasks in progress | DAT-11/12/13 are Live verified; DAT-14/15/16/17/20 are Live verified at their stated scopes. `DAT-20` establishes reliable deep-book observability and quiet-time skips, not rare-event predictability. **`SIG-21` is the exact next research task: pre-register the deep-book-anomaly → future-price-response event study and its sample/power gate before inspecting outcomes.** INF-02/05/07/09 are tested; INF-06 is blocked on exact approved destinations for 8 Drive-hosted files. CON-02/03/04/05/06/07/09 are tested; CON-01/08 and the Dhan slice of CON-05 retain live evidence. DAT-03/04/07 are tested; DAT-05/06 are dry-run verified; DAT-08 is dropped; DAT-09 planning/pooling is tested with the final strike-band/connection-count owner choice still open |

**Immediate next action:** write and commit the `SIG-21` pre-registration **before** inspecting any
