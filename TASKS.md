...
| D42 | **Shaurya's NSE data lives on the IEX storage server and is partitioned by IST trading date** | **Directed by Aryan 2026-08-21.** The physical archive is the verified SMB share `//Aryan@172.20.10.38/Aryan`, rooted at `/Volumes/Aryan/NSE`, with one `YYYY-MM-DD/` partition containing `raw/`, `metadata/`, `indexes/`, and `derived/`. Normal Dhan, option-chain, and live-surface DAT capture defaults to the daily `raw/` lane. Capture fails closed if the expected SMB share is absent or different; an output outside the archive requires the separately named controlled-test override. This settles physical placement and daily partitioning only. It does **not** silently settle DAT-19's codec, file granularity, seek index, or warm/cold implementation. The 2026-08-21 full-session capture started before this configuration and remains in its original local run root; changing an open writer's destination was deliberately not attempted. |
...
| ANL-07 | **Stable held-out executable-IV option-dislocation monitor.** Approved by Aryan 2026-08-20 after deciding that an in-sample option-minus-surface residual is circular. For every fresh option, build a five-fold strike-held-out eSSVI reference that removes both CE and PE at the held-out strike, uses a matching future or robust parity from other fresh strikes, and converts held-out IV to Black-76 fair price. Require continuous past-only IV uncertainty, forward/asynchrony IV stress, executable ask IV for cheap / bid IV for rich, positive explicit after-cost rupee edge, dated tick/lot metadata and one displayed lot, BH-FDR at 1%, an exact leave-strike refit, and two consecutive valid frames. **Owner amendment 1:** because both legs are market-derived, every episode carries exact signed target-option and held-out reference-market contributions. **Owner amendment 2:** raw five-second folds are never actionable: each fold uses a causal 60-second-half-life parameter smoother, twelve consecutive fits, a twelve-frame smoothed-IV range no larger than 0.10 volatility points, and raw-smoothed/exact-smoothed agreement no larger than 0.10 points. **Owner amendment 3:** absolute rupee price is no longer the identification state. Hard moneyness/liquidity residual buckets are prohibited and replaced by same-expiry/type Gaussian-weighted nearest-neighbour IV residuals over continuous log-moneyness and log-relative spread. The entry executable IV, fair-IV boundary, continuous uncertainty calibration and after-cost IV target are frozen; only target executable-IV movement that closes that target for two frames is corrected. Forward-driven price movement, reference/mixed movement, band/stability loss and missing data remain invalidated or censored as specified. Current rupee edge is an execution overlay, and every episode reports a frozen-entry-delta executable-quote markout explicitly labelled as a scenario proxy. Dashboard/API expose the complete stability, continuous uncertainty, IV lifecycle, rupee overlay and markout state. This is a read-only estimated classification — not latent true value, arbitrage, fill, realised P&L, signal, or order authority. Frozen spec: `docs/SURFACE-MISPRICING-SPEC-2026-08-20.md`. | New; reuses SUR-02/05/08 and ANL-03/05 shell | ANL-03, ANL-05, SUR-02, SUR-05, SUR-08, CON-01, CON-05, CON-06, CON-07 | **Owner-amendment 3 Implemented and focused-tested 2026-08-20; full and replacement-live verification pending.** `mispricing.py` owns causal per-fold smoothing, continuous past-only IV uncertainty, executable-IV direction and lifecycle, exact agreement, frozen IV correction, invalidation/re-warm and frozen-delta markout; `surface_feed.py` preserves fit-decision smoothing and source staleness time; `dashboard.py` renders continuous uncertainty support, IV contributions, rupee execution edge and markout proxy. Current evidence: 56 focused tests pass, strict mypy and Ruff are clean on changed source/tests, and a six-key/20,000-sample performance gate handled 450 continuous-neighbour queries in 0.18 seconds. |
...
| `MODULE_SPEC.md` | **Drafted 2026-08-18; extended through D36 on 2026-08-19** — root index plus 13 per-component specifications under `docs/module-spec/`; all non-dropped task IDs map to stable `REQ-*` rows with code/test/output targets. `DAT.md` and `SIG.md` bind D36's full-session capture and selection-aware replication; dropped `DAT-08`/`EXE-07/08` intentionally have no requirement |
