...
- [ ] **ACTIVE — run `X-OFI-DAT20-03` now.** Isolate price-keyed OFI over the frozen
  5-window × 7-depth × 5-return-horizon grid; rank the complete 175 cells by honest OOS R²;
  report OFI-only, state baseline + OFI, nested marginal-depth, past-mirror and same-window arms.
  Design: `docs/OFI-PREDICTIVE-SCAN-SPEC-2026-08-19.md`.
...
**Immediate next action (D35):** complete `X-OFI-DAT20-03` on the two permanently exploratory
`DAT-20` tapes, publish the full 175-cell grid and plain-English read-out, then restore the next
action to the enforced `--sig21-calibration` profile below. Do not interpret this one-session scan
as confirmation or modify `H-SIG21` from it.

**Next action after the OFI scan (D31–D32):** run the enforced `--sig21-calibration` profile for the same
