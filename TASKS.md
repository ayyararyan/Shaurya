...
| ANL-07 | **Stable held-out executable option-dislocation monitor.** Approved by Aryan 2026-08-20 after deciding that an in-sample option-minus-surface residual is circular. For every fresh option, build a five-fold strike-held-out eSSVI reference that removes both CE and PE at the held-out strike, uses a matching future or robust parity from other fresh strikes, and converts held-out IV to Black-76 fair price. Require an empirical past-only fair band, forward/asynchrony stress, executable ask for cheap / bid for rich, positive explicit after-cost edge, dated tick/lot metadata and one displayed lot, BH-FDR at 1%, an exact leave-strike refit, and two consecutive valid frames. **Owner amendment 1:** because both legs are market-derived, every episode carries exact signed target-option and held-out reference-market contributions. **Owner amendment 2:** raw five-second folds are never actionable: each fold uses a causal 60-second-half-life parameter smoother, twelve consecutive fits, a twelve-frame smoothed-IV range no larger than 0.10 volatility points, and raw-smoothed/exact-smoothed agreement no larger than 0.10 points. The entry benchmark is frozen; only target-quote movement that closes its frozen after-cost gap for two frames is corrected, while reference/mixed/stability-led disappearances are invalidated and missing data are censored; invalidation forces a fresh twelve-frame warm-up. Dashboard/API expose the complete stability state and all outcomes. The superseded 30-second/six-frame live calibration admitted the same 1 September 24450 CE twice and invalidated both reference-led, so neither is retained as an opportunity result. This is a read-only estimated classification — not latent true value, arbitrage, fill, signal, or order authority. Frozen spec: `docs/SURFACE-MISPRICING-SPEC-2026-08-20.md`. | New; reuses SUR-02/05/08 and ANL-03/05 shell | ANL-03, ANL-05, SUR-02, SUR-05, SUR-08, CON-01, CON-05, CON-06, CON-07 | **Owner-amendment 2 Implemented and Tested 2026-08-20; calibrated replacement live verification pending.** `mispricing.py` owns causal per-fold smoothing, stability gates, exact agreement, fixed-entry target correction, invalidation and re-warm; `surface_feed.py` uses fit decision time for temporal smoothing while preserving source staleness time; `dashboard.py` renders raw/smoothed IV diagnostics, frozen target requirements and corrected/invalidated/censored outcomes. Verification: 47 focused tests and 554 full-suite tests pass; whole-repository Ruff, strict mypy on 59 source files, compileall and diff checks are clean. |
