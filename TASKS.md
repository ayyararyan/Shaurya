...
| DAT-17 | **Measure the delivery cadence of every depth tier we may consume, so `D27`'s binding bound is known rather than assumed.** `DAT-16` established that depth20 is a fixed 500 ms publication clock (2.00 book states/s, burst-gap p05/p50/p95 496/501/506 ms) and that Quote/Full is *not* a metronome (1.22–1.66 distinct-timestamp updates/s, dispersed gaps). Two tiers remain uncharacterised: **(i) the 5-level depth block carried inside the Full packet** — is it republished on the Quote/Full clock or on its own; and **(ii) the 200-level channel**, whose cadence has never been grouped by receive timestamp and which `DAT-13` showed is additionally subject to a first-subscription throttle, so its effective per-instrument cadence beyond the first instrument may be far worse than its nominal one. Method is `DAT-16`'s and requires no new code: group tape rows by distinct receive timestamp, take successive differences, report burst rate, rows per burst, gap percentiles, and the fraction of bursts carrying a top-of-book price change. **Must also state which tier binds when two are consumed together**, since `D27` makes the slower one govern. Also worth resolving, though it may not be identifiable from tape alone: whether each cadence is a Dhan publication interval, broker-side aggregation, or an NSE-side snapshot — only one of those is negotiable | `docs/live-evidence/DAT-16-2026-08-19.md` (method); existing tapes for depth200; a fresh healthy capture needed for a clean depth200 sample | DAT-16, DAT-10, DAT-13 | **Scheduled 2026-08-19 ~11:28 IST, not yet started.** Aryan asked for the depth200 operational bounds to be measured and explicitly required it to be sequenced *after* the running `ANL-03` Opus agent, not concurrently. Queued as gateway cron job `b5ccc369` (`shaurya-dat17-depth200-bounds`), schedule kind `on-exit`: a watcher (`workspace/bin/anl03-gate-watch`) polls `subagent_runs` for run `d0bce3c8-a6b7-49dc-a0b7-ff311efbd33c` and fires the job once that run ends, or at 14:45 IST, whichever comes first. Payload is an isolated `openai/gpt-5.6-sol` agent turn, `timeoutSeconds` 5400, Telegram delivery to Aryan on success, error and timeout alike. Brief adds two things beyond the row below: the observed per-socket depth200 instrument ceiling (the depth200 analogue of `DAT-11`'s 50), and first- versus later-instrument cadence under `DAT-13`'s throttle, since a solo-instrument cadence number is not the operational bound. Requires market hours for the depth200 sample; the existing depth200 tapes are a 366-row 45 s run and one reconnect-heavy cross-tier run, neither adequate. The 5-level question may be answerable from retained tape offline |
...
| `MODULE_SPEC.md` | **Drafted 2026-08-18; extended through D26 and DAT-16 on 2026-08-19** — root index plus 13 per-component specifications under `docs/module-spec/`; all 110 non-dropped task IDs mapped exactly once to stable `REQ-*` rows with code/test/output targets |
...
| Tasks in progress | DAT-11/12/13 are Live verified; DAT-14 is Live verified for two front-month futures/depth20/one morning window; DAT-15 is Live verified for the retained eight-tape sample with the stressed cross-tier and missing option/midday coverage explicit; DAT-16 live-verifies the 500 ms depth20 snapshot cadence at its stated scope. INF-02/05/07/09 are tested; INF-06 is blocked on exact approved destinations for 8 Drive-hosted files. CON-02/03/04/05/06/07/09 are tested; CON-01/08 and the Dhan slice of CON-05 retain live evidence. DAT-01/DAT-02/DAT-10-16 are live-verified at their stated scopes; DAT-03/04/07 are tested; DAT-05/06 are dry-run verified; DAT-08 is dropped; DAT-09 planning/pooling is tested with the final strike-band/connection-count owner choice still open |

**Immediate next action:** DAT-11 through DAT-16 market-hours work is landed. Use the measured
