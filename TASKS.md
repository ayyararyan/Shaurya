...
| ANL-07 | **Surface-relative executable option-mispricing monitor.** Approved by Aryan 2026-08-20 after deciding that an in-sample option-minus-surface residual is circular. For every fresh option, build a five-fold strike-held-out eSSVI reference that removes both CE and PE at the held-out strike, uses a matching future or robust parity from other fresh strikes, and converts held-out IV to Black-76 fair price. Require an empirical past-only fair band, forward/asynchrony stress, executable ask for cheap / bid for rich, positive explicit after-cost edge, dated tick/lot metadata and one displayed lot, BH-FDR at 1%, an exact leave-strike refit, and two consecutive valid frames. Correction needs two valid non-positive-net frames; stale/missing/failed/unsupported observations are censored rather than called corrected. Dashboard/API must show active and corrected/censored rows with market/fair/band, gross/net ticks, net/lot, IV residual, quote age, first/close time and duration. **Owner amendment 1, 2026-08-20:** because both legs are market-derived, every episode now carries signed target-option and held-out reference-market contributions whose sum equals the net gap closed, plus target-option-led/reference-market-led/mixed attribution and the separate closing gate. This is a read-only estimated classification — not latent true value, arbitrage, fill, signal, or order authority. Frozen spec: `docs/SURFACE-MISPRICING-SPEC-2026-08-20.md`. | New; reuses SUR-02/05/08 and ANL-03/05 shell | ANL-03, ANL-05, SUR-02, SUR-05, SUR-08, CON-01, CON-05, CON-06, CON-07 | **Implemented and Tested 2026-08-20; not yet Dry-run or Live verified.** `src/shaurya/analytics/mispricing.py` owns fresh strike cross-fitting, robust held-out parity, empirical uncertainty/FDR, explicit cost and metadata assumptions, exact confirmation, stateful episode lifecycle and exact gap-close accounting; `surface_feed.py` freezes each frame's monitor state; `dashboard.py` adds full-width Active and Recently corrected/censored tables with entry gap, target contribution, reference contribution and net gap closed; live CLI takes dated master tick/lot metadata and exposes all policy values. Synthetic acceptance covers IV inversion, warm-up, cheap exact-confirmation, two-frame activation/correction, target/reference identity and attribution, one-lot rejection, censoring and no-order imports. Owner-amendment evidence: 35 focused tests and 549 full-suite tests pass; Ruff, strict mypy (59 source files), and compileall are clean. Real retained-tape replay and live-chain verification remain pending, so no empirical count or opportunity claim is made. |
