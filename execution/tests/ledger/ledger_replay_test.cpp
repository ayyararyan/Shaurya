...
JsonObject risk_payload() {
  return {{"decision_id", JsonValue(std::string("00000000-0000-4000-8000-000000000301"))},
          {"configuration_digest", JsonValue(std::string(64, 'd'))}};
}
}
...
int main() {
  ...
  const auto path = root / "ledger.jsonl";
  {
    ...
    static_cast<void>(ledger.append(event(
        "00000000-0000-4000-8000-000000000103", "risk_approved", risk_payload(),
        std::string(kIntent), std::string(kOrder))));
    static_cast<void>(ledger.append(event(
    ...
    static_cast<void>(ledger.append(event(
        ...
        {{"update_id", JsonValue(std::string("ack-1"))},
         {"broker_order_id", JsonValue(std::string("paper-1"))}},
        std::string(kIntent), std::string(kOrder))));
    static_cast<void>(ledger.append(event(
        "00000000-0000-4000-8000-000000000106", "partially_filled",
        {{"update_id", JsonValue(std::string("fill-1"))},
         {"cumulative_filled_quantity", JsonValue(JsonInteger{"20"})}},
        std::string(kIntent), std::string(kOrder))));
  }
  ...
  require_test(state.positions.at(kInstrument) == 20, "position");
  require_test(state.route_positions.at("route-123") == 20, "route-token position");
  require_test(state.orders.begin()->second.cumulative_filled_quantity == 20, "fill");
  ...
  const auto ambiguous_path = root / "ambiguous.jsonl";
  {
    ...
    static_cast<void>(ledger.append(event(
        ...
        std::string(kIntent), std::string(kOrder))));
    static_cast<void>(ledger.append(event(
        "00000000-0000-4000-8000-000000000113", "risk_approved", risk_payload(),
        std::string(kIntent), std::string(kOrder))));
    static_cast<void>(ledger.append(event(
    ...
    static_cast<void>(ledger.append(event(
        "00000000-0000-4000-8000-000000000123", "risk_approved", risk_payload(),
        std::string(kIntentTwo), std::string(kOrderTwo))));
    static_cast<void>(ledger.append(event(
  ...
  const std::array crash_events{
      ...
      event("00000000-0000-4000-8000-000000000142", "mapping_validated", mapping_payload(),
            std::string(kIntent), std::string(kOrder)),
      event("00000000-0000-4000-8000-000000000143", "risk_approved", risk_payload(),
            std::string(kIntent), std::string(kOrder)),
