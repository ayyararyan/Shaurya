"""Option-chain universe selection for surface fitting and for feed-ceiling measurement.

The Quote/Full channel (`RequestCode` 21) carries a five-level book per instrument, which is
what the eSSVI fitter needs. Its per-subscription instrument ceiling is *not* documented
reliably — Dhan's published 5,000 was disproved for the 20-level depth channel on
2026-08-19 (real ceiling 50) — so this module keeps the requested universe explicit and
ordered, letting a capture report which of the requested instruments actually produced
packets.
"""
...
class ChainUniverse:
    """An ordered subscription universe plus the selection it came from."""
    ...
    def instruments(self) -> tuple[DhanInstrumentMapping, ...]:
        """Futures first: the forward source must not be the item a ceiling truncates."""
...
def select_chain_universe(
    ...
    *,
    ...
    """Pick both option wings around ``spot_reference`` plus any matching traded future.

    Options are taken nearest-the-money first and interleaved across expiries, so that a
    subscription ceiling truncates the far wings rather than deleting a whole maturity.
    """
