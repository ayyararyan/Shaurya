...
| ID | Requirement | Status | Code location | Test / acceptance evidence |
|---|---|---|---|---|
| `D50-DATA-01` | Completed 2026-08-21 DAT tape, exact hash, five-second causal anchor clock | **Empirically verified** | `cli/nonlinear_ofi_state.py`; `cadence_observations` | 439,949 source rows; 4,472 cadence rows; tape SHA recorded |
| `D50-GEO-01` | Eight causal geometry features derived only from existing C8 predictors | **Empirically verified after correction** | `nonlinear_ofi_state.py:_geometry` | finite support and terminal `[0,1]` invariants; regression test |
| `D50-MODEL-01` | M0–M4 exact frozen horse race plus constant-shrinkage falsifier | **Empirically verified** | `build_calibration_artifact` | 9,457 test model-horizon outcomes; compact result artifact |
| `D50-CAUSAL-01` | Mature labels only, delayed Kalman updates, purged forward gate labels | **Empirically verified** | `_rolling_prediction`, `_kalman_path`, `fit_gate` | terminal causal flags true; test selection flag false |
| `D50-VAL-01` | Train/validation/test chronological split; test never selects hyperparameters | **Empirically verified** | `split_masks`; estimator selection paths | 1,670/996/1,351 train/validation/test rows plus purges |
| `D50-OUT-01` | Full artifact, compact result and plain-English report | **Accepted** | D50 CLI; D50 result and report documents | strict JSON read-back; artifact SHA `1b47c034…f36b` |
...
| `D51-MODEL-01` | Train-only median imputation, explicit missing indicators, centering/scaling; validation/test apply only | **Implemented, tested** | `ModelTransform`, `fit_model_transform`, `apply_model_transform` | train/held-out isolation and explicit missing-indicator synthetic tests |
| `D51-MODEL-02` | Transparent elastic net saves all ordered coefficients/intercept/transform and retains correlated inputs | **Implemented, tested** | `ElasticNetModel`, `fit_elastic_net`, `predict_model` | near-copy correlated-predictor retention and prediction test |
...
| `D51-STAB-01` | Aggregate supplied walk-forward results separately by complete cluster/model: selection frequency, median delta, positive-fold fraction and direction consistency | **Implemented, tested** | `ClusterFoldStabilityResult`, `aggregate_cluster_stability_selection` | stable 20-session/five-fold fixture; unstable-direction fixture; no fold constructor exists |
| `D51-STAB-02` | Frozen 20-session/five-fold/70% promotion gates plus positive median, no dominance, mirror and supplied economic-value guards | **Implemented, tested** | `StabilityPromotionConfig`, `ClusterModelStabilitySelection` | stable signal passes; one-session, unstable-sign, single-session-dominance and past-mirror failures carry exact reasons |
| `D51-STAB-03` | One-session evidence is always exploratory; publish session and volatility/spread/time-phase coverage plus leave-one-session-out diagnostics | **Implemented, tested** | `PromotionStatus`, regime coverage and dominance fields | one-day fixture returns `exploratory_insufficient_sessions`; stable fixture covers all declared regimes |
