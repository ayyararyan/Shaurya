...
| Mispricing correction duration and gap-close trace | Deterministically derived | Runs from first qualifying breach until two valid non-positive-net frames; stale/missing/failed paths are censored. Signed target-option and held-out reference-market contributions add exactly to the net gap closed. Both legs are market-derived; target-option-led/reference-market-led/mixed is endpoint accounting, not causal identification. |
...
- Selects market data through `CON-10` DAT handles and consumes DAT replay/follow/live. Dashboards own
  presentation and analytics state only; they do not own broker credentials/sockets, capture
  manifests, raw-tape discovery or file-tail parsing (D43).
...
| REQ-ANL-06 | Provide a read-only **dynamic order-flow-imbalance dashboard**: the frozen `X-OFI-HORSERACE-DAT20-05` predictor apparatus (`M0`-`M6`) refitted continuously on a walk-forward split over the full 7 x 5 x 5 = 175-cell grid, driven either by replay of a pinned tape or by a read-only tail of a tape another process is writing. **It opens no socket and holds no order path.** Binding display constraints, approved by Aryan 2026-08-20: raw OOS R2 **and** the past-mirror placebo-benchmarked increment are both always visible; block and accumulated walk-forward scores are separately labelled and not interchangeable; the multiplicity chance-expectation count and leader churn are on screen; negative and insufficient-support cells stay visible; brick colour is reserved for the past-mirror-exceeds-future condition. Presentation inherits the `ANL-05` shell. The `M0`-`M6` formulas are called from `src/shaurya/signals/`, never restated. Exploratory and non-confirmatory: it is not a signal, carries no `SIG-21` authority, and does not alter `H-SIG21`. **Amended 2026-08-20 by `docs/OFI-DASHBOARD-SPEC-AMENDMENT-1-2026-08-20.md`, approved by Aryan:** the hero band ranks by **future incremental OOS R2 over `M0` among cells that pass the placebo guard**, not by the placebo-benchmarked difference. The difference is unbounded in its past term, so a collapsed placebo manufactured a leader — observed live at +128.31 pp benchmarked against a +5.64 pp future increment and a −122.67 pp past mirror. The benchmarked increment is retained in full as the guard, still published and still displayed; it is no longer the sort key. The magnitude ramp and sign glyph follow the same ranking statistic so colour and rank cannot disagree; brick remains reserved for past-mirror-exceeds-future. No estimand, threshold or cell membership changed. | ANL-06, ANL-05, ANL-03, D32 | `src/shaurya/analytics/ofi_dashboard.py`, `src/shaurya/cli/ofi_dashboard.py`, reusing `src/shaurya/signals/ofi_horserace.py` | Walk-forward partition and one-way-ratchet tests; future-only/past-only leakage probe; torn-line tail probe; offline horse-race parity probe; theme and field-parity audit; per-cell JSONL record and compact summary |
| REQ-ANL-07 | Add the approved read-only **surface-relative executable option-mispricing monitor** below ANL-03. Fair value must be strike-held-out rather than the target's in-sample residual; eligibility requires fresh valid BBO, held-out support and dated tick/lot metadata; uncertainty is past-only and explicit; cheap uses ask, rich uses bid; positive after-cost edge must survive BH-FDR, an exact leave-strike refit and two-frame persistence. Correction needs two valid non-positive-net frames; unavailable data is censored. **Owner amendment 5, 2026-08-21:** use a 120-second causal smoothing half-life and six-fit smoother warm-up; remove the rolling stability window, max-minus-min IV gate and post-invalidation stability-history rebuild. Current raw-smoothed and exact-smoothed agreement must remain within 0.50 volatility points. **Owner amendment 6, 2026-08-21 (presentation only):** visible active/recent rows show entry market/fair/gap, current-or-exit market/fair/gap, market movement, fair-boundary movement and state/outcome; the complete diagnostic row remains in `/api/state` and `/api/history`. No order, signal, arbitrage, fill or latent-true-value claim. | ANL-07, ANL-03, SUR-02/05/08, CON-01/05/06/07 | `src/shaurya/analytics/mispricing.py`, `surface_feed.py`, `dashboard.py`, `cli/surface_dashboard.py` | `tests/test_surface_mispricing.py`, `tests/test_anl03_dashboard.py`; frozen `docs/SURFACE-MISPRICING-SPEC-2026-08-20.md`; focused/full static and test gates; live verification pending |
| REQ-ANL-08 | Resolve read-only dashboard inputs as DAT dataset handles and use DAT replay/follow/live transport. Surface live mode consumes DAT's authenticated bounded localhost canonical-row fan-out, fits only after new input on the three-second clock, and filters shared multi-underlying handles before calibration. Surface and OFI dashboards cannot import Dhan adapters, read data credentials, construct capture manifests/writers or implement raw-file tails. Display and estimator semantics remain independent of transport. | ANL-08, D43, CON-10, DAT-22 | `src/shaurya/cli/surface_dashboard.py`; `src/shaurya/cli/ofi_dashboard.py`; DAT public access API | Architecture boundary test; replay/follow/live parity; no-new-input fit gate; shared active-handle and multi-underlying isolation integration |
| REQ-ANL-09 | Execute D38, D39 and D40 intraday on newline-complete, hash-identified prefixes of the active DAT dataset and publish findings into ANL-06 as they complete. D38 publishes touch diagnostics; D40 publishes the exact seven-horizon C8/M10/h1=10 curve; D39 publishes all 600 fixed-target cells incrementally. Each view states prefix freshness and that overlapping prefixes are exploratory/non-independent. Post-close remains final verification. HTTP serves the last complete ANL-06 frame while a refit runs. No socket, credentials, signal or order path. | ANL-09, D44, ANL-06, ANL-08, D38, D39, D40 | `src/shaurya/analytics/live_ofi_studies.py`; `src/shaurya/cli/live_ofi_studies.py`; `src/shaurya/analytics/ofi_dashboard_server.py` | `LIVE-*` acceptance matrix; active-DAT adoption, torn-prefix, per-cell callback, partial-state, D40-curve, rendering/API and refit-responsiveness tests; growing production dashboard check |
...
- Broker acquisition, credentials, capture lifecycle, raw tape discovery, or consumer-owned file
  parsing/tailing.
...
- ANL-06 is Implemented, Tested and Dry-run verified under
  `docs/OFI-DASHBOARD-SPEC-2026-08-20.md` (scan `X-OFI-DASHBOARD-2026-08-20`). D43 moves its
  replay/follow transport behind a DAT handle without changing the estimator. Production growing
  follow remains a separate live-verification gate; its authority boundary is exploratory only.
- ANL-09 is Implemented, Tested and live-operation verified under
  `docs/D38-D39-D40-LIVE-AMENDMENT-2026-08-21.md`. Its growing-prefix worker and dashboard/API
