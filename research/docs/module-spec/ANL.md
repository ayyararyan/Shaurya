...
- Selects market data through `CON-10` DAT handles and consumes DAT replay/follow. Dashboards own
...
| REQ-ANL-08 | Resolve read-only dashboard inputs as DAT dataset handles and use DAT replay/follow transport. Surface and OFI dashboards cannot import Dhan adapters, read data credentials, construct capture manifests/writers or implement raw-file tails. Display and estimator semantics remain byte-for-byte independent of transport. | ANL-08, D43, CON-10, DAT-22 | `src/shaurya/cli/surface_dashboard.py`; `src/shaurya/cli/ofi_dashboard.py`; DAT public access API | Architecture boundary test; replay/follow parity; shared active-handle integration |
