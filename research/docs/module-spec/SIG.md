...
Under `D31`, `docs/sig-claims/H-SIG21.md` is the binding execution registration. The registration
and outcome-blind construction were pushed and remotely verified at
`f2cf65011d02882191b5cfda566c1024119964d7` before the response/power/inference code was built.
The construction module may not import or emit prices, returns, labels or response objects. The
separate outcome gate remains closed until a complete numeric power artifact is pushed from five
post-registration calibration sessions. The first outcome sample is a two-sided
informativeness/discovery test on 20 later full sessions. A direction or horizon learned there
cannot be promoted; it must be frozen under a new hypothesis ID and confirmed on subsequently
collected tape.
