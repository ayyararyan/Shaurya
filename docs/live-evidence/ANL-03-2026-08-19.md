...
## Where the staleness actually lives: moneyness-resolved instrument ages (MEASURED)

Raised by Aryan 2026-08-19 12:51 IST during the continuing afternoon run: *is the flagged
staleness only the deep out-of-the-money wings, with the at-the-money strikes updating
continuously?* The dashboard's `worst_instrument_id` names one instrument but does not
publish the age distribution, so the question was answered from the run's own canonical tape
rather than from the API.

Method: full scan of `data/live-captures/anl03-live/sha-20260819T063412.584779Z-0a555c5b/`
(1,194,878 rows, 452 instruments, 2.3 s), taking the maximum `receive_ts` per
`instrument_id`, and measuring each instrument's age against the newest row in the tape
(12:52:38 IST). Log-moneyness uses the same per-expiry forwards the live fit used
(24,115.15 / 24,147.70 / 24,238.40).

Out-of-the-money options only — these are the 225 quotes the fit consumes:

| Bucket | n | age p50 | age p95 | age max | ≥3 s | ≥60 s |
|---|---:|---:|---:|---:|---:|---:|
| ATM, \|k\| < 0.5% | 15 | 0.67 s | 1.21 s | 1.24 s | **0** | **0** |
| 0.5–1.5% | 29 | 0.40 s | 1.27 s | 1.48 s | **0** | **0** |
| 1.5–3% | 42 | 0.40 s | 1.27 s | 1.47 s | **0** | **0** |
| 3–5% | 59 | 0.59 s | 5.36 s | 346.4 s | 4 | 1 |
| 5–9% | 80 | 1.39 s | 140.0 s | 396.5 s | 26 | 7 |
| All OTM | 225 | 0.65 s | 39.1 s | 396.5 s | 30 | 8 |

Both traded futures were 0.07–0.08 s old.

**Finding.** The staleness is confined to the outer wings and is driven by a minority of
individually illiquid strikes, not by a dead region. Every one of the 86 instruments inside
±3% of the forward reprinted within 1.5 s; none was stale on any threshold. Beyond 3% the
distribution develops a long right tail, and beyond 5% one instrument in three exceeds the
3 s instrument threshold. The stalest contributors at this instant were
`2026-09-01 25750 CE` (k = +6.42%, 396 s) and `2026-09-01 25250 CE` (k = +4.46%, 346 s).
Because `ESSVISurface.surface_timestamp` is `min(receive_timestamp)` over contributing quotes
(`essvi.py:482`), the whole surface inherits the single worst wing quote, which is why the
session-level surface age ran at p50 400 s / p95 852 s / max 968 s while the at-the-money
core was never more than ~1.2 s old.

**Interpretation, stated so it is not read as a defect.** Where a wing quote is minutes old
there is generally no new information to be had — the option is not printing because nothing
is happening at that strike. The surface is as fresh as the market is at that strike. What
the measurement does establish is that "surface age" and "age of the tradeable core of the
surface" are two different objects (§7.2), and that the current dashboard reports the first.

**Decision (Aryan, 2026-08-19 12:51 IST):** continue the current behaviour unchanged for
`ANL-03`. No change to the fit interval, the strike window, or the `SUR-07` staleness
definition. This is scoped to the watching dashboard under `D19`.

**Two consequences that ride on this definition and remain open for later fronts** — recorded
here so they are not rediscovered:

1. `is_stale` reads `True` for roughly 58% of a post-warm-up session. `RSK-05` lists "stale
   surface (`SUR-07`)" among its kill-switch triggers; wired against this definition as-is, it
   would trip more or less permanently.
2. The `SUR-07` temporal smoother engaged in only 3 of 45 sampled snapshots — it refuses a
   non-increasing surface timestamp, and a min-over-quotes timestamp stalls whenever a wing has
   not reprinted. Since `SUR-07` makes raw fits fail closed for quoting, there is currently no
   quote-ready surface in live conditions. Harmless for watching; a prerequisite for any
   quoting path.

Neither is a change to `ANL-03`. Both attach to whichever task first consumes the surface for
a decision rather than for display.

---

## Erratum — full-session storage projection (added 2026-08-19)

The statement above that a full session at this width would be roughly 15 GB used the former
09:15–15:30 session length. NSE equity derivatives close at 15:40 from 2026-08-03; scaling the
same observed byte rate to 23,100 seconds gives approximately **15.63 GB**. The live dashboard
run, its captured evidence, and its surface-quality findings are unchanged.
