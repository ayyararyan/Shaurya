...
- The pre-existing personal-Mac `~/Documents/Secrets/` to office-Mac
  `~/Documents/Market-Making-Secrets/` one-way mirror already represents this credential.
  As of 2026-08-28 the previously separate `kotak.env` and `dhan.env` were merged into one
  file, `broker.env`, so both Kotak and Dhan credentials rest in a single place; the old
  files were renamed to `*.retired-2026-08-28` rather than deleted. On the office Mac, the
  mirror is mode `700`, `broker.env` is mode `600`, and `scripts/dat09_concurrency_probe.py`
  reads that external file via its `--credentials` flag (only the `DHAN_*` keys are parsed;
  the `KOTAK_*` keys alongside them are ignored). This does not extend to the EC2-side
  Kotak-only files (`/etc/market-making-live/kotak.env`, `/etc/alo-smm/alo-smm.env`), which
  stay separate, narrower, root-owned deployed copies on that production host.
